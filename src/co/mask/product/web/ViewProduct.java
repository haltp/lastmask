...
public class ViewProduct implements Command {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) {
		// ViewProduct.do 처리 메소드
		
		  ProductDao dao = new ProductDao(); ProductVo vo = new ProductVo();
		  vo.setProductNum(Integer.parseInt(request.getParameter("productNum")));
		  dao.select(vo); request.setAttribute("vo", vo);
		
		return "view/product/viewProductForm";
	}
